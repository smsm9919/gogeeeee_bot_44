...
EMERGENCY_RSI_HOT = 72
EMERGENCY_RSI_COLD = 28
EMERGENCY_ATR_SPIKE = 1.6
EMERGENCY_HARVEST = 0.60
...
OPPOSITE_LOCK_AFTER_CLOSE = True
    ...
    if not MODE_LIVE:
        class _Paper:
            ...
            def fetch_ohlcv(self, *a, **k): 
                raise RuntimeError("Paper mode يحتاج مصدر بيانات حقيقي.")
            def fetch_ticker(self, *a, **k): 
                raise RuntimeError("ticker unavailable in Paper stub")
            def fetch_order_book(self, *a, **k): 
                return {"bids": [[0, 0]], "asks": [[0, 0]]}
            def fetch_balance(self, *a, **k): 
                return {"total": {"USDT": 100.0}, "free": {"USDT": 100.0}}
    ...
    try:
        if os.path.exists(STATE_FILE):
            with open(STATE_FILE, "r", encoding="utf-8") as f: 
                d = json.load(f)
            set_state(d.get("state", {}))
            compound_pnl = float(d.get("compound_pnl", 0.0))
...
def _interval_seconds(iv: str) -> int:
    iv = iv.lower()
    if iv.endswith("m"):
        return int(float(iv[:-1])) * 60
    elif iv.endswith("h"):
        return int(float(iv[:-1])) * 3600
    elif iv.endswith("d"):
...
def compute_next_sleep(df):
    ...
    tf = _interval_seconds(INTERVAL)
    if left <= 10 or (tf - left) <= JUST_CLOSED_WIN: 
...
def safe_qty(q):
    q = _round_amt(q)
    if q <= 0: 
        logging.warning(f"qty invalid after normalize → {q}")
    return q
...
def compute_indicators(df: pd.DataFrame):
    if len(df) < max(ATR_LEN, RSI_LEN, ADX_LEN) + 2:
        return {
            "rsi": 50.0, "plus_di": 0.0, "minus_di": 0.0, "dx": 0.0, 
            "adx": 0.0, "atr": 0.0, "st": None, "st_dir": 0, 
            "ema9": None, "ema20": None, "ema9_slope": 0.0
        }
    ...
    for i in range(1, len(df)):
        ...
        if prev_dir == 1:
            ub = min(ub, prev_st)
            st_now = lb if cur > ub else ub
            dir_now = 1 if cur > ub else -1
        else:
            lb = max(lb, prev_st)
            st_now = ub if cur < lb else lb
    ...
    st_dir_series = pd.Series(dirv[1:], index=df.index[1:]).reindex(df.index, method='pad').fillna(0)
...
def compute_rf_closed(df):
    if len(df) < RF_PERIOD + 3:
        return {
            ...
            "filter": float(df["close"].iloc[-1]),
            "hi": float(df["close"].iloc[-1]),
            "lo": float(df["close"].iloc[-1]),
            "fdir": 0
        }
    ...
    fdir = dfilt.apply(lambda x: 1 if x > 0 else (-1 if x < 0 else 0)).ffill().fillna(0)
    ...
    for i in range(1, len(src)):
        if bool(src_gt_f.iloc[i] and upward.iloc[i] > 0): 
            cond_prev.iloc[i] = 1
        elif bool(src_lt_f.iloc[i] and downward.iloc[i] > 0): 
            cond_prev.iloc[i] = -1
        else: 
            cond_prev.iloc[i] = cond_prev.iloc[i-1]
    ...
    try:
        ...
        for i in range(len(d) - 2, max(len(d) - SMC_OB_LOOKBACK, 1), -1):
            ...
            if abs(c - o) >= 1.2 * max(atr_now, 1e-12):
                ...
                ob = {"side": side, "bot": min(o, c), "top": max(o, c)}
                break
        
        fvg = None
        for i in range(len(d) - 3, max(len(d) - SMC_FVG_LOOKBACK, 2), -1):
            phh = float(d["high"].iloc[i-1])
            pll = float(d["low"].iloc[i-1])
            ch = float(d["high"].iloc[i])
            cl = float(d["low"].iloc[i])
            
            if cl > phh and (cl - phh) <= SMC_FVG_MAX_ATR * max(atr_now, 1e-12):
                fvg = {"type": "BULL_FVG", "bottom": phh, "top": cl}
                break
            if ch < pll and (pll - ch) <= SMC_FVG_MAX_ATR * max(atr_now, 1e-12):
                fvg = {"type": "BEAR_FVG", "bottom": ch, "top": pll}
                break
...
def fib_targets_from_swings(entry_px, swing_low, swing_high, side):
    if not (entry_px and swing_low and swing_high) or swing_high <= swing_low: 
        return []
    ...
    if side == "long":
        ...
        for ext in FIB_EXTS:
            t = swing_low + base * ext
            pct = (t - entry_px) / entry_px * 100.0
            if pct > 0: 
                targets.append(round(pct, 2))
    else:
        base = swing_high - swing_low
        for ext in FIB_EXTS:
            t = swing_high - base * ext
            pct = (entry_px - t) / entry_px * 100.0
            if pct > 0: 
                targets.append(round(pct, 2))
...
def in_golden_pocket(px, a, b):
    ...
    gp_min = lo + (hi - lo) * FIB_GP_MIN
    gp_max = lo + (hi - lo) * FIB_GP_MAX
...
def true_break_confirmed(df, level, above=True):
    ...
    if above: 
        return ((close - level) / level) * 10000.0 >= TRUE_BREAK_MIN_CLOSE_BPS
    else:     
...
def explosion_detector(df, ind, prev_ind):
    ...
    return {
        "flag": flag, 
        "range_atr": round(st.get("range_atr", 0.0), 2), 
        "adx_delta": round(adx_acc, 2), 
        "body_ratio": round(st.get("body_ratio", 0.0), 2),
...
def _position_params_for_close():
    if BINGX_POSITION_MODE == "oneway":
        ...
        side = state.get("side")
        return {"positionSide": ("LONG" if side == "long" else "SHORT"), "reduceOnly": True}
...
def close_partial(frac, reason):
    ...
    if state["qty"] - qty_close < min_unit: 
        qty_close = safe_qty(max(0.0, state["qty"] - min_unit))
    ...
    if MODE_LIVE:
        try: 
            ...
            logging.error(f"close_partial: {e}")
            return
    ...
    set_state({
        "qty": state["qty"] - qty_close,
        "scale_outs": state["scale_outs"] + 1,
...
def reset_after_full_close(reason):
    ...
    if OPPOSITE_LOCK_AFTER_CLOSE and state.get("side"):
        ...
        print(colored(f"🔐 Opposite-Signal Lock → waiting for next {lock_side.upper()} signal", "yellow"))
...
def close_market_strict(reason):
    ...
    if exch_qty <= 0:
        if state.get("open"): 
            reset_after_full_close(f"{reason} (flat)")
        return
    ...
    try:
        ...
        logging.error(f"strict close: {e}")
...
def ema_harvest(ind, px):
    ...
    if ema9 is not None and ((side == "long" and px < ema9) or (side == "short" and px > ema9)):
        close_partial(EMA9_PARTIAL, "EMA9 touch")
        acted = "EMA9"
    
    if ema20 is not None and ((side == "long" and px < ema20) or (side == "short" and px > ema20)):
        close_partial(EMA20_PARTIAL, "EMA20 break")
        acted = (acted or "") + "+EMA20"
...
def breakout_failure_guard(df, px, ind, smc_snap):
    ...
    if side == "long" and hh and near_bps(px, hh, FAIL_PROX_BPS):
        if ind.get("adx", 0.0) < MIN_TREND_ADX_HOLD or not true_break_confirmed(df, hh, above=True):
            ...
            if state["_fail_votes"] == 1:
                close_partial(FAIL_DEFEND_PARTIAL, "HH failure defend")
                state["breakeven"] = state.get("breakeven") or state.get("entry")
            ...
            if state["_fail_votes"] >= FAIL_CLOSE_AFTER_VOTES:
                ...
                if allow: 
                    close_market_strict("HH_BREAK_FAILURE_CONFIRMED")
    ...
    if side == "short" and ll and near_bps(px, ll, FAIL_PROX_BPS):
        if ind.get("adx", 0.0) < MIN_TREND_ADX_HOLD or not true_break_confirmed(df, ll, above=False):
            state["_fail_votes"] += 1
            if state["_fail_votes"] == 1:
                close_partial(FAIL_DEFEND_PARTIAL, "LL failure defend")
                state["breakeven"] = state.get("breakeven") or state.get("entry")
...
def emergency_layer(ind, prev_ind, px):
    if not (EMERGENCY_ENABLED and state["open"]): 
        return
    ...
    if atr_prev > 0 and atr_now >= atr_prev * EMERGENCY_ATR_SPIKE and adx >= EMERGENCY_ADX_MIN:
        if (state["side"] == "long" and rsi <= EMERGENCY_RSI_COLD) or (state["side"] == "short" and rsi >= EMERGENCY_RSI_HOT):
            ...
            close_partial(EMERGENCY_HARVEST, "EMERGENCY harvest")
            state["breakeven"] = state.get("breakeven") or state.get("entry")

def breakout_signal(df, ind, prev_ind):
    if not BREAKOUT_ENABLED or len(df) < BREAKOUT_LOOKBACK + 2: 
        return None
    ...
    if not (adx >= BREAKOUT_ADX and atr_prev > 0 and atr_now >= atr_prev * BREAKOUT_ATR_SPIKE): 
        return None
    
    highs = df["high"].iloc[-BREAKOUT_LOOKBACK:-1].astype(float)
    lows = df["low"].iloc[-BREAKOUT_LOOKBACK:-1].astype(float)
    
    if len(highs) and price > highs.max(): 
        return "BUY"
    if len(lows) and price < lows.min():   
        return "SELL"
...
def post_open_diagnostics(df, ind, smc, traps, expl):
    ...
    msgs = []
    if smc.get("fvg"): 
        msgs.append("FVG near — راقب ملء الفجوة")
    if smc.get("ob"):  
        msgs.append(f"OB {smc['ob']['side']} zone — احتمال ارتداد")
    if smc.get("eqh"): 
        msgs.append("EQH overhead — احذر فخ قمة")
    if smc.get("eql"): 
        msgs.append("EQL below — احذر فخ قاع")
    if traps["bull_trap"] > 0.5 or traps["bear_trap"] > 0.5: 
        msgs.append("Trap probability HIGH — tighten")
    if expl["flag"]: 
        msgs.append("Explosion momentum — harvest بسرعة لو انعكس")
