# -*- coding: utf-8 -*-
"""
RF Futures Bot — RF-LIVE ONLY (BingX Perp via CCXT) + ORCHESTRA + SMC + STATE TRACKING
• Entry: Range Filter — LIVE candle only (RF فقط)
• Post-entry brain (Orchestra + SMC): EQH/EQL + FVG + OB + BOS/CHOCH + PD + Sweeps
• Dynamic TP + Breakeven + ATR-trailing + Wick/Impulse harvesting + ONE-SHOT
• Strict close مع تحقق + Dust/Final-chunk guard + Opposite-signal wait
• NEW: تتبّع حالة المنظومة (داخل OB/FVG/EQ) + عرضها في اللوج و/metrics
"""
...
RISK_ALLOC = float(os.getenv("RISK_ALLOC", 0.60))   # 60% × 10x
...
SMC_ENABLED         = True
OB_LOOKBACK         = 80
OB_DISP_MULT_ATR    = 1.2
OB_TOLERANCE_BPS    = 12.0
BOS_LOOKBACK_SWINGS = 8
PD_MID_METHOD       = "fib50"
SWEEP_WICK_PCT      = 55.0
CHOCH_NEED_BREAK    = True
...
def time_to_candle_close(df: pd.DataFrame) -> int:
    ...
    left = max(0, next_close_ms - now_ms); return int(left/1000)
...
def _find_swings(df: pd.DataFrame, left:int=2, right:int=2):
    if len(df) < left+right+3: return [None]*len(df), [None]*len(df)
    ...
    for arr, is_high in ((ph, True),(pl, False)):
        for i, price in enumerate(arr):
            ...
            for j in range(max(0,i-10), min(len(arr), i+10)):
                pj = arr[j]
                if pj is not None and abs(pj - price) <= tol: local.append(pj)
            if len(local) >= 2:
                (eqh_candidates if is_high else eql_candidates).append(max(local) if is_high else min(local))
...
def _pd_mid(high, low, method="fib50"):
    ...
    return low + (high-low)*0.62 if method=="fib62" else (high+low)/2.0
...
def _bos_choch(df_closed: pd.DataFrame, ph, pl):
    ...
    if ph[lh] is not None and last_close > ph[lh]:
        bos = "up";  choch = bool(plv is not None and df_closed["close"].iloc[ll] < pl[plv])
    if pl[ll] is not None and last_close < pl[ll]:
        bos = "down"; choch = choch or bool(phv is not None and df_closed["close"].iloc[lh] > ph[phv])
    return {"bos":bos, "choch":choch}
def _find_order_blocks(df_closed: pd.DataFrame, atr_now: float):
    bull = None; bear = None; n = len(df_closed)
    if n < 10 or atr_now<=0: return {"bull":bull, "bear":bear}
    for i in range(n-3, max(3, n-OB_LOOKBACK)-1, -1):
        o = float(df_closed["open"].iloc[i]); c = float(df_closed["close"].iloc[i])
        h = float(df_closed["high"].iloc[i]); l = float(df_closed["low"].iloc[i])
        if c < o:
            fwd_high = float(df_closed["high"].iloc[i+1:i+4].max())
            disp = fwd_high - h
            if disp >= OB_DISP_MULT_ATR*atr_now:
                bull = {"time": int(df_closed["time"].iloc[i]), "low": l, "high": o, "type":"BULL_OB"}; break
    for i in range(n-3, max(3, n-OB_LOOKBACK)-1, -1):
        o = float(df_closed["open"].iloc[i]); c = float(df_closed["close"].iloc[i])
        h = float(df_closed["high"].iloc[i]); l = float(df_closed["low"].iloc[i])
        if c > o:
            ...
            disp = l - fwd_low
            if disp >= OB_DISP_MULT_ATR*atr_now:
                bear = {"time": int(df_closed["time"].iloc[i]), "low": c, "high": h, "type":"BEAR_OB"}; break
...
def _liquidity_sweep(df: pd.DataFrame, ph, pl):
    ...
    hs = [v for v in ph if v is not None]; ls = [v for v in pl if v is not None]
    eqh = max(hs) if hs else None; eql = min(ls) if ls else None
    if eqh and h>eqh and c<eqh and upper_pct>=SWEEP_WICK_PCT: return {"type":"SWEEP_HIGH", "level":eqh}
    if eql and l<eql and c>eql and lower_pct>=SWEEP_WICK_PCT: return {"type":"SWEEP_LOW", "level":eql}
    return None
def build_structure_snapshot(df_full: pd.DataFrame, ind: dict):
    ...
    if SMC_ENABLED:
        ...
        highs = [v for v in ph if v is not None]; lows  = [v for v in pl if v is not None]
        hi = max(highs) if highs else None; lo = min(lows) if lows else None
        mid = _pd_mid(hi, lo, PD_MID_METHOD) if hi and lo else None
        last_close = float(df_closed["close"].iloc[-1]) if len(df_closed) else None
        if mid and last_close: smc["pd"] = "premium" if last_close>mid else "discount"
        atr_now = float(ind.get("atr") or 0.0); smc["ob"] = _find_order_blocks(df_closed, atr_now)
...
STATE = {
    ...
    # NEW: متابعة حالة المنظومة/المناطق
    "orchestra_mode": "FLAT",
    "zone_flags": {"in_bull_ob": False, "in_bear_ob": False,
                   "in_fvg": False, "fvg_type": None,
                   "near_eqh": False, "near_eql": False},
    "engine_active": False,
    "last_action": None,
}
compound_pnl = 0.0
...
def open_market(side, qty, price):
    ...
    STATE.update({
        ...
        "struct": None, "rejection_votes": 0, "hold_votes": 0,
        "last_level_tag": None, "last_reject_bar_ts": None, "confirm_queue": 0,
        "orchestra_mode": "NORMAL", "engine_active": True,
        "zone_flags": {"in_bull_ob": False, "in_bear_ob": False, "in_fvg": False,
                       "fvg_type": None, "near_eqh": False, "near_eql": False},
        "last_action": "OPEN"
    })
    ...
    while attempts < CLOSE_RETRY_ATTEMPTS:
        try:
            ...
            if left_qty <= 0:
                ...
                _reset_after_close(reason, prev_side=side); return
            ...
            last_error = e; logging.error(f"close_market_strict attempt {attempts+1}: {e}")
            attempts += 1; time.sleep(CLOSE_VERIFY_WAIT_S)
...
def _reset_after_close(reason, prev_side=None):
    ...
    STATE.update({
        ...
        "struct": None, "rejection_votes": 0, "hold_votes": 0,
        "last_level_tag": None, "last_reject_bar_ts": None, "confirm_queue": 0,
        "orchestra_mode": "FLAT", "engine_active": False,
        "zone_flags": {"in_bull_ob": False, "in_bear_ob": False, "in_fvg": False,
                       "fvg_type": None, "near_eqh": False, "near_eql": False},
        "last_action": f"AFTER_CLOSE:{reason}"
    })
...
def close_partial(frac, reason):
    ...
    STATE["last_action"] = f"PARTIAL:{reason}"
...
def _candle_stats(df):
    ...
    return {"range":rng, "body":body, "body_pct": (body/rng*100.0 if rng>0 else 0.0),
            "upper_pct": (upper/rng*100.0 if rng>0 else 0.0),
            "lower_pct": (lower/rng*100.0 if rng>0 else 0.0),
            "bull": c>o, "bear": c<o}
def _vol_ratio(df):
    if len(df)<21: return 1.0
    v = float(df["volume"].iloc[-1]); vma = df["volume"].iloc[-21:-1].astype(float).mean()
    return (v/max(vma,1e-9)) if vma>0 else 1.0
def near_level(px, lvl, bps):
    try: return abs((px-lvl)/lvl)*10000.0 <= bps
    except Exception: return False

def _update_zone_flags(px, st):
    """تحديث أعلام الحالة + وضع المنظومة (للعرض فقط)."""
    flags = {"in_bull_ob": False, "in_bear_ob": False,
             "in_fvg": False, "fvg_type": None,
             "near_eqh": False, "near_eql": False}
    mode = "NORMAL" if STATE["open"] else "FLAT"
    if not st: STATE["zone_flags"]=flags; STATE["orchestra_mode"]=mode; return

    eqh, eql, fvg, smc = st.get("eqh"), st.get("eql"), st.get("fvg"), (st.get("smc") or {})
    ob = smc.get("ob") or {}; bull_ob = ob.get("bull"); bear_ob = ob.get("bear")

    # EQ proximity
    flags["near_eqh"] = bool(eqh and near_level(px, eqh, LEVEL_PROX_BPS))
    flags["near_eql"] = bool(eql and near_level(px, eql, LEVEL_PROX_BPS))

    # OB zones
    if bull_ob and _in_zone(px, bull_ob["low"], bull_ob["high"], OB_TOLERANCE_BPS):
        flags["in_bull_ob"] = True; mode = "OB-MITIGATION"
    if bear_ob and _in_zone(px, bear_ob["low"], bear_ob["high"], OB_TOLERANCE_BPS):
        flags["in_bear_ob"] = True; mode = "OB-MITIGATION"

    # FVG zone
    if fvg:
        low = min(fvg["bottom"], fvg["top"]); high = max(fvg["bottom"], fvg["top"])
        if low <= px <= high:
            flags["in_fvg"] = True; flags["fvg_type"]=fvg["type"]; mode = "FVG-REPRICING"

    # EQ test overrides (للتوضيح)
    if flags["near_eqh"] and STATE.get("side")=="long": mode = "EQ-TEST"
    if flags["near_eql"] and STATE.get("side")=="short": mode = "EQ-TEST"

    STATE["zone_flags"] = flags
    STATE["orchestra_mode"] = mode

def orchestra_post_entry(df, ind, info):
    if not (ORCHESTRA_ENABLED and STATE["open"] and STATE["qty"]>0):
        STATE["engine_active"] = bool(STATE["open"])
        return
    px   = info["price"]; entry = STATE["entry"]; side = STATE["side"]
    rr   = (px - entry)/entry*100*(1 if side=="long" else -1)
    ...
    if trap:
        ...
        if rr >= BREAKEVEN_AFTER and STATE.get("breakeven") is None: STATE["breakeven"]=entry
        if atr>0:
    ...
    if side=="long" and cndl.get("upper_pct",0)>=WICK_HARVEST_PCT: close_partial(0.25, f"UPPER-WICK {cndl['upper_pct']:.0f}%")
    if side=="short" and cndl.get("lower_pct",0)>=WICK_HARVEST_PCT: close_partial(0.25, f"LOWER-WICK {cndl['lower_pct']:.0f}%")
    ...
    if targeted_level:
        ...
        is_reject = False
        if side=="long":
            is_reject = (float(df["high"].iloc[-1])>targeted_level and px<targeted_level and cndl["upper_pct"]>=40.0 and _vol_ratio(df)>=VOL_SPIKE_X and adx_soft)
        else:
            is_reject = (float(df["low"].iloc[-1])<targeted_level and px>targeted_level and cndl["lower_pct"]>=40.0 and _vol_ratio(df)>=VOL_SPIKE_X and adx_soft)
        if is_reject:
    ...
    if SMC_ENABLED and smc:
        ...
        ob  = smc.get("ob") or {}; bull_ob = ob.get("bull"); bear_ob = ob.get("bear"); sweep = smc.get("sweep")
        ...
        if (side=="long" and bos=="up") or (side=="short" and bos=="down"): STATE["hold_votes"] = min(STATE["hold_votes"]+1, 3)
        ...
        if side=="long" and bear_ob and _in_zone(px, bear_ob["low"], bear_ob["high"], OB_TOLERANCE_BPS) and cndl.get("upper_pct",0)>=40.0:
            close_partial(0.25, "Opposite OB reject"); STATE["rejection_votes"] = min(STATE["rejection_votes"]+1, 3)
    ...
    # ---- ONE-SHOT decision ----
    targeted_level = (STATE["struct"]["eqh"] if side=="long" else STATE["struct"]["eql"])
    ...
    if targeted_level and STATE.get("rejection_votes",0) >= REJECT_NEED_VOTES and rr >= ONE_SHOT_MIN_PCT and last_conf_bar_ok:
        ...
        close_market_strict("ONE_SHOT_LAST_LEVEL"); return

    if STATE.get("confirm_queue",0) > 0:
        STATE["confirm_queue"] = max(0, STATE["confirm_queue"]-1)

    STATE["engine_active"] = True  # brain running
...
def pretty_snapshot(bal, info, ind, spread_bps, reason=None, df=None):
    ...
    print(f"   🎯 ENTRY: RF-LIVE ONLY  •  ⏱️ closes_in ≈ {left_s}s")
    ...
    print(colored(f"   🧱 OB: BULL[{fmt((bull_ob or {}).get('low'))}-{fmt((bull_ob or {}).get('high'))}]  BEAR[{fmt((bear_ob or {}).get('low'))}-{fmt((bear_ob or {}).get('high'))}]", "yellow"))

    # NEW: عرض حالة المنظومة/المناطق
    flags = STATE.get("zone_flags", {})
    mode  = STATE.get("orchestra_mode")
    engine= "ACTIVE" if STATE.get("engine_active") else "IDLE"
    print(colored(f"   🛰️ Engine={engine}  Mode={mode}  | Flags: in_bull_ob={flags.get('in_bull_ob')} in_bear_ob={flags.get('in_bear_ob')} in_fvg={flags.get('in_fvg')}({flags.get('fvg_type')}) near_EQH={flags.get('near_eqh')} near_EQL={flags.get('near_eql')}", "cyan"))
    if STATE.get("last_action"): print(colored(f"   🧾 last_action: {STATE['last_action']}", "magenta"))
...
def trade_loop():
    ...
    while True:
        try:
            ...
            info = rf_signal_live(df); ind  = compute_indicators(df)
...
def home():
    ...
    return f"✅ RF-LIVE Bot — {SYMBOL} {INTERVAL} — {mode} — RF LIVE only — Dynamic TP — Strict Close — ORCHESTRA+SMC — STATE TRACKING"
@app.route("/metrics")
def metrics():
    st = STATE.get("struct") or {}; smc = st.get("smc") or {}; ob  = smc.get("ob") or {}
    return jsonify({
        "symbol": SYMBOL, "interval": INTERVAL, "mode": "live" if MODE_LIVE else "paper",
        "leverage": LEVERAGE, "risk_alloc": RISK_ALLOC, "price": price_now(),
        "state": {
            "open": STATE["open"], "side": STATE["side"], "entry": STATE["entry"],
            "qty": STATE["qty"], "pnl": STATE["pnl"], "trail": STATE["trail"],
            "breakeven": STATE["breakeven"], "tp1_done": STATE["tp1_done"],
            "highest_profit_pct": STATE["highest_profit_pct"],
            "profit_targets_achieved": STATE["profit_targets_achieved"],
            "orchestra_mode": STATE.get("orchestra_mode"),
            "engine_active": STATE.get("engine_active"),
            "zone_flags": STATE.get("zone_flags"),
...
def health():
    return jsonify({
        ...
        "entry_mode": "RF_LIVE_ONLY", "wait_for_next_signal": wait_for_next_signal_side,
        "orchestra_enabled": ORCHESTRA_ENABLED, "smc_enabled": SMC_ENABLED,
        "engine_active": STATE.get("engine_active"), "orchestra_mode": STATE.get("orchestra_mode"),
        "zone_flags": STATE.get("zone_flags"), "last_action": STATE.get("last_action")
    }), 200
...
if __name__ == "__main__":
    ...
    print(colored(f"ENTRY: RF ONLY  •  FINAL_CHUNK_QTY={FINAL_CHUNK_QTY}  •  ORCHESTRA+SMC=ON  •  STATE TRACKING=ON", "yellow"))
