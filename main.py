...
SCALE_IN_MAX_STEPS = getenv("SCALE_IN_MAX_STEPS", 3, int)
SCALE_IN_STEP_PCT = getenv("SCALE_IN_STEP_PCT", 0.20, float)
ADX_STRONG_THRESH = getenv("ADX_STRONG_THRESH", 28, float)
RSI_TREND_BUY = getenv("RSI_TREND_BUY", 55, float)
RSI_TREND_SELL = getenv("RSI_TREND_SELL", 45, float)
TRAIL_MULT_STRONG = getenv("TRAIL_MULT_STRONG", 2.0, float)
TRAIL_MULT_MED = getenv("TRAIL_MULT_MED", 1.5, float)
TRAIL_MULT_CHOP = getenv("TRAIL_MULT_CHOP", 1.0, float)
...
def should_scale_in(candle_info: dict, ind: dict, current_side: str) -> tuple:
    ...
    if state["scale_ins"] >= SCALE_IN_MAX_STEPS:
        return False, "Max scale-in steps reached"
    ...
    if adx < ADX_STRONG_THRESH:
        return False, f"ADX {adx:.1f} < {ADX_STRONG_THRESH}"
    
    # RSI direction confirmation
    if current_side == "long" and rsi < RSI_TREND_BUY:
        return False, f"RSI {rsi:.1f} < {RSI_TREND_BUY}"
    if current_side == "short" and rsi > RSI_TREND_SELL:
        return False, f"RSI {rsi:.1f} > {RSI_TREND_SELL}"
    ...
    if candle_strength < 2:
        return False, f"Weak candle pattern: {candle_info.get('name_en', 'NONE')}"
    ...
    if candle_info.get("pattern") in strong_patterns:
        return True, f"Strong {candle_info.get('name_en')} pattern + ADX {adx:.1f}"
    
    return False, f"Moderate pattern: {candle_info.get('name_en', 'NONE')}"
...
def advanced_position_management(candle_info: dict, ind: dict):
    ...
    # Scale-in check
    should_scale, scale_reason = should_scale_in(candle_info, ind, current_side)
    if should_scale:
        scale_in_position(SCALE_IN_STEP_PCT, scale_reason)
        return "SCALE_IN"
    ...
    trail_mult = get_trail_multiplier(ind)
...
def snapshot(bal,info,ind,spread_bps,reason=None, df=None):
    ...
    if state["open"]:
        ...
        print(f"   📊 TP1_done={state['tp1_done']}  Breakeven={fmt(state['breakeven'])}")
...
def trade_loop():
    ...
    while True:
        try:
            ...
            # Smart profit (trend-aware) with advanced management
...
def home():
    ...
    return f"✅ RF Bot — {SYMBOL} {INTERVAL} — {mode} — {STRATEGY.upper()} — ADVANCED"
