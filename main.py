# -*- coding: utf-8 -*-
"""
DOGE RF + Council Futures Bot — BingX Perp via CCXT
Strategy order:
  1) Council (bottom/top + zones): primary entries & exits
  2) Fallback: Range Filter (TradingView-like) on CLOSED candle only
Post-entry management:
  - Dynamic TP ladder + Breakeven + ATR trailing (ratchet lock)
  - Strict close verification + final-chunk guard
  - Defensive handling on opposite RF while in trade
HTTP:
  - /         : status line
...
# Range Filter (TV-like) — CLOSED candle ONLY
RF_SOURCE   = os.getenv("RF_SOURCE", "close")
RF_PERIOD   = int(os.getenv("RF_PERIOD", 20))
RF_MULT     = float(os.getenv("RF_MULT", 3.5))
RF_HYST_BPS = float(os.getenv("RF_HYST_BPS", 6.0))  # لتقليل التذبذب
ENTRY_FROM_RF = True
...
# حراسة المخلفات
FINAL_CHUNK_QTY   = float(os.getenv("FINAL_CHUNK_QTY", 50.0))  # ≤ 50 DOGE
...
# إيقاف فتح صفقات عندما ADX ضعيف
PAUSE_ADX_THRESHOLD = float(os.getenv("PAUSE_ADX_THRESHOLD", 18.0))

# تصويتات إشارة معاكسة أثناء صفقة
...
def rf_signal_closed(df: pd.DataFrame):
    """Range Filter signal based on the LAST CLOSED candle (no live tick)."""
    if len(df) < RF_PERIOD + 3:
        return {"time": int(time.time()*1000), "price": None, "long": False, "short": False,
                "filter": None, "hi": None, "lo": None}
    d = df.iloc[:-1]  # use CLOSED bars only
    ...
    return {
        "time": int(d["time"].iloc[-1]), "price": p_now,
...
class Council:
    ...
    def votes(self, df: pd.DataFrame, ind: dict, info: dict, zones: dict):
        ...
        if self.rsi_neutral_min <= rsi <= self.rsi_neutral_max:
            if float(df["close"].iloc[-1])>float(df["open"].iloc[-1]): b+=1; rb.append("RSI_neutral_up")
        ...
        # RF (closed) confirmation vote only (لا يفتح وحده هنا)
    ...
    def update(self, df: pd.DataFrame, ind: dict, rf_closed_info: dict):
        ...
        return {"entry":entry,"exit":exit_,"debug":{
            "zones":zones, "votes":{"buy":bv,"buy_reasons":br,"sell":sv,"sell_reasons":sr},
...
compound_pnl = 0.0
wait_for_next_signal_side = None  # بعد الإغلاق لا يدخل إلا من الاتجاه المعاكس
...
def trade_loop():
    ...
    while True:
        try:
            ...
            rf_c = rf_signal_closed(df)     # الشمعة المغلقة فقط
            ...
            # دفاع عند عكس RF أثناء الصفقة
            if STATE["open"]:
                opp = (STATE["side"]=="long" and rf_c["short"]) or (STATE["side"]=="short" and rf_c["long"])
                if opp:
                    defensive_on_opposite_rf(ind, {"price": px or rf_c["price"], **rf_c})
            ...
            if council_decision["entry"]:
                ...
                sig = "buy" if rf_c["long"] else "sell"

            # انتظار الاتجاه المعاكس بعد الإغلاق
            if not STATE["open"] and sig and reason is None:
                if wait_for_next_signal_side and sig != wait_for_next_signal_side:
                    ...
                    if qty>0 and (px or rf_c["price"]):
                        if open_market(sig, qty, px or rf_c["price"]):
...
def home():
    ...
    return f"✅ DOGE Council+RF Closed — {SYMBOL} {INTERVAL} — {mode} — TP/BE/Trail — Strict Close — FinalChunk={FINAL_CHUNK_QTY}DOGE"
